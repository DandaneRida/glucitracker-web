class GluciApp {
  ...
  calculateDoses(mealType) {
    ...
    // 3b. Ajouter 2u si glucides > 120g
    if (totalGlucides > 120) {
      doseTotale += 2;
    }

    meal.doseTotale = doseTotale;

    // 4. Correction 3-4h après (si glycémie > 1.6 g/L - valeur cible)
    const glycemieApres = parseFloat(meal.glycemie_apres) || 0;
    meal.correctionTroisHeures = 0;
    if (glycemieApres > 1.6) {
      meal.correctionTroisHeures = sensitivity > 0 ? (glycemieApres - 1.6) / sensitivity : 0;
    }
  ...
  ensureMealFields() {
    const defaultMealTemplate = {
      ...
      glycemie_avant: null,
      glycemie_apres: null,
      heure_glycemie_avant: '',
      heure_glycemie_apres: '',
      resucrage: 0,
      doseRepas: 0,
      doseCorrection: 0,
      doseTotale: 0,
    ...
    meals.forEach(mealType => {
      if (!this.mealsData[mealType]) {
        ...
        if (meal.heure_glycemie_avant === undefined) meal.heure_glycemie_avant = '';
        if (meal.heure_glycemie_apres === undefined) meal.heure_glycemie_apres = '';
        if (meal.doseTotalePrise === undefined) meal.doseTotalePrise = '';
        if (meal.correctionTroisHeuresPrise === undefined) meal.correctionTroisHeuresPrise = '';
        ...
        <div class="meal-section">
          ...
          <div style="display: flex; gap: 10px;">
            <input type="number" 
              ...
              class="meal-input" style="flex: 2;">
            <input type="time"
              value="${meal.heure_glycemie_avant || ''}"
              onchange="app.updateMealValue('${mealType}', 'heure_glycemie_avant', this.value)"
              class="meal-input" style="flex: 1;" title="Heure de la mesure">
            <button onclick="document.getElementById('comment-avant-${mealType}').style.display = 'block'" class="btn-secondary" style="padding: 0 12px;" title="Ajouter un commentaire">💬</button>
          </div>
        ...
        <div class="calculation-section">
          ...
          <div style="background: white; padding: 15px; border-radius: 8px; border-left: 5px solid #667eea; margin: 15px 0;">
            ...
            <small style="display: block; color: #999; font-size: 11px; margin-top: 8px;">
              ...
              ${this.mealsData[mealType].aliments.reduce((s,i)=>s+i.glucides_totaux,0) > 120 ? '🍽️ +2u (glucides > 120g)' : ''}
            </small>
            <div style="margin-top: 10px;">
              <label style="font-size: 12px; color: #667eea; font-weight: 600;">✏️ Dose réellement prise (u)</label>
              <input type="number"
                value="${meal.doseTotalePrise || ''}"
                placeholder="Ex: 8"
                min="0" max="100" step="0.5"
                onchange="app.updateMealValue('${mealType}', 'doseTotalePrise', this.value)"
                class="meal-input" style="margin-top: 4px;">
            </div>
          </div>

          <div class="calculation-input-group">
            <div>
              <label>⏰ Correction 3-4h après</label>
              <span class="calculation-result">${meal.correctionTroisHeures.toFixed(2)} u</span>
              <small>si glyc. après > 1.6 g/L</small>
            </div>
          </div>
          <div style="margin-top: 8px; padding: 10px; background: #fff; border-radius: 8px; border: 1px solid #e2e8f0;">
            <label style="font-size: 12px; color: #667eea; font-weight: 600;">✏️ Dose correction 3-4h réellement prise (u)</label>
            <input type="number"
              value="${meal.correctionTroisHeuresPrise || ''}"
              placeholder="Ex: 2"
              min="0" max="100" step="0.5"
              onchange="app.updateMealValue('${mealType}', 'correctionTroisHeuresPrise', this.value)"
              class="meal-input" style="margin-top: 4px;">
          </div>
        </div>
        ...
        <div class="meal-section">
          ...
          <div style="display: flex; gap: 10px;">
            <input type="number" 
              ...
              class="meal-input" style="flex: 2;">
            <input type="time"
              value="${meal.heure_glycemie_apres || ''}"
              onchange="app.updateMealValue('${mealType}', 'heure_glycemie_apres', this.value)"
              class="meal-input" style="flex: 1;" title="Heure de la mesure">
            <button onclick="document.getElementById('comment-apres-${mealType}').style.display = 'block'" class="btn-secondary" style="padding: 0 12px;" title="Ajouter un commentaire">💬</button>
          </div>
...
<table>
  <thead>
    <tr>
      <th rowspan="2">Repas</th>
      <th colspan="2">Glycémie Avant</th>
      <th rowspan="2">Glucides (g)</th>
      <th rowspan="2">Dose Repas</th>
      <th rowspan="2">Correction</th>
      <th colspan="2">Dose Totale</th>
      <th colspan="2">Glycémie 3-4h</th>
      <th colspan="2">Corr. 3-4h</th>
    </tr>
    <tr>
      <th>Valeur</th>
      <th>Heure</th>
...
`;
  ...
  exportReport() {
    try {
      ...
      // Entête tableau (12 colonnes)
      const headers = ['Repas', 'Gly.Av', 'H.Av', 'Glucides', 'D.Repas', 'D.Corr', 'D.Tot.Calc', 'D.Tot.Prise', 'Gly.Ap', 'H.Ap', 'C3h.Calc', 'C3h.Prise'];
      const colWidths = [22, 14, 12, 14, 14, 14, 16, 16, 14, 12, 16, 14];
  ...
  buildTableRowsHTML(notesArray) {
    return ['petit_dejeuner', 'dejeuner', 'diner'].map(mealType => {
      ...
      return `<tr>
        ...
        <td>${meal.glycemie_avant || '-'}${addNote(meal.commentaire_glycemie_avant, labels[mealType] + ' (Glyc. Avant)')}</td>
        <td style="color:#888;font-size:10px;">${meal.heure_glycemie_avant || '-'}</td>
        <td><strong>${totalGlucides.toFixed(4)}g</strong>${totalGlucides > 120 ? '<br><small style="color:#e65100">+2u</small>' : ''}</td>
        <td>${meal.doseRepas.toFixed(2)}u</td>
        <td>${meal.doseCorrection.toFixed(2)}u</td>
        <td><strong>${meal.doseTotale.toFixed(2)}u</strong>${resucrage}</td>
        <td style="color:${meal.doseTotalePrise ? '#1b5e20' : '#aaa'};font-weight:bold;">${meal.doseTotalePrise ? meal.doseTotalePrise + 'u' : '—'}</td>
        <td>${meal.glycemie_apres || '-'}${addNote(meal.commentaire_glycemie_apres, labels[mealType] + ' (Glyc. Après)')}</td>
        <td style="color:#888;font-size:10px;">${meal.heure_glycemie_apres || '-'}</td>
        <td>${meal.correctionTroisHeures.toFixed(2)}u</td>
        <td style="color:${meal.correctionTroisHeuresPrise ? '#1b5e20' : '#aaa'};font-weight:bold;">${meal.correctionTroisHeuresPrise ? meal.correctionTroisHeuresPrise + 'u' : '—'}</td>
      </tr>`;
    }).join('');
  }
  ...
  buildTableRowsForPDF(notesArray) {
    ...
    meals.forEach(mealType => {
      ...
      rows.push([
        mealLabel,
        meal.glycemie_avant !== null ? `${meal.glycemie_avant}` : '-',
        meal.heure_glycemie_avant || '-',
        `${totalGlucides.toFixed(2)}${totalGlucides > 120 ? '+2u' : ''}`,
        `${meal.doseRepas.toFixed(2)}u`,
        `${meal.doseCorrection.toFixed(2)}u`,
        `${meal.doseTotale.toFixed(2)}u${resucrageStr}`,
        meal.doseTotalePrise ? `${meal.doseTotalePrise}u` : '-',
        meal.glycemie_apres !== null ? `${meal.glycemie_apres}` : '-',
        meal.heure_glycemie_apres || '-',
        `${meal.correctionTroisHeures.toFixed(2)}u`,
        meal.correctionTroisHeuresPrise ? `${meal.correctionTroisHeuresPrise}u` : '-'
      ]);
    });
